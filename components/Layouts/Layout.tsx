...
export default function Layout({
  ...
  const [activeVideos, setActiveVideos] = useState<VideoStatus[]>([]);
  ...
  // ตรวจสอบสถานะวิดีโอที่กำลัง queued หรือ running
  useEffect(() => {
    if (!user?.userId) return;
    
    const checkActiveVideos = async () => {
      try {
        const res = await fetch(`/api/videos/active-status?userId=${user.userId}`);
        const data = await res.json();
        
        if (data.success && data.videos) {
          const active = data.videos.map((v: any) => ({
            videoId: v.videoId,
            status: v.status,
    ...
    checkActiveVideos();
    const interval = setInterval(checkActiveVideos, 5000);
    return () => clearInterval(interval);
  }, [user?.userId]);
      ...
      <header className="fixed top-0 w-full bg-gray-50 shadow-md z-50 h-16">
        <div className="max-w-7xl 2xl:max-w-full mx-auto flex items-center justify-between px-6 2xl:px-12 h-full">
          ...
          <nav className="hidden md:flex space-x-8 text-gray-700 font-medium h-full items-center">
            ...
            {/* แสดงสถานะวิดีโอที่กำลังประมวลผล */}
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-lg shadow-sm">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm text-amber-700 font-medium">
                    {activeVideos.length} งานกำลังประมวลผล
                  </span>
                </div>
                <button
                  onClick={() => {
                    const oldestVideo = activeVideos[activeVideos.length - 1];
                    router.push(`/create-video?id=${oldestVideo.videoId}`);
                  }}
                  className="text-xs text-amber-700 hover:text-amber-900 font-medium underline decoration-amber-400 hover:decoration-amber-600 ml-1 transition-colors"
                  title="คลิกเพื่อดูสถานะงานที่เก่าที่สุด"
                >
                  ดูรายละเอียด
                </button>
              </div>
...
//                 <button
//                   onClick={() => {
//                     // ลิงก์ไปที่งานที่เก่าที่สุด (งานสุดท้ายใน array เพราะเรียงจากใหม่ -> เก่า)
//                     const oldestVideo = activeVideos[activeVideos.length - 1];
//                     router.push(`/create-video?id=${oldestVideo.videoId}`);
//                   }}
//                   className="text-xs text-amber-700 hover:text-amber-900 font-medium underline decoration-amber-400 hover:decoration-amber-600 ml-1 transition-colors"
