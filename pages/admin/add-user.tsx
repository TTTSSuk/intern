...
// ฟังก์ชันสุ่ม userId: "u" + ตัวเลขสุ่ม 6-8 หลัก
const generateUserId = () => {
  const length = Math.floor(Math.random() * 3) + 6; // 6-8 หลัก
...
// ฟังก์ชันสุ่ม password: ตัวอักษร + ตัวเลข + เครื่องหมายพิเศษ 1 ตัว
...
export default function AddUserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ generate userId & password อัตโนมัติ
    ...
    try {
      const res = await fetch("/api/admin/users", {
        ...
        body: JSON.stringify({ userId: newUserId, password: newPassword, name }),
      });
      ...
      if (res.ok) {
        ...
        setName("");
        setMessage({ text: "เพิ่มผู้ใช้เรียบร้อย", success: true });
      } else {
    ...
    <AdminLayout>
      <div className="max-w-md mx-auto bg-white p-8 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* ช่อง userId และ password ไม่ต้องให้ admin กรอก */}
          <p className="text-sm text-gray-500">
            ⚡ UserID และ Password จะถูกสร้างอัตโนมัติเมื่อเพิ่มผู้ใช้
          </p>
