...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  interface FileDoc {
    ...
    executionId: string;
    userId: string;
    jobType?: string; // 🔥 เพิ่ม field นี้
    originalName: string;
    clips: Clip[];
    status?: string;
  ...
  try {
    ...
    if (video) {
        ...
        if (fileDoc) {
            const jobType = fileDoc.jobType || 'normal'; // 🔥 ตรวจสอบ jobType
            
            // 🔥 หัก token เฉพาะ jobType === 'normal'
            if (jobType === 'normal') {
                ...
                console.log(`✅ [${jobType}] Deducted 1 token from user ${fileDoc.userId}`);
            } else {
                console.log(`ℹ️ [${jobType}] No token deduction needed`);
            }
        }
    }
