      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="text-center my-6">
          <p className="text-2xl text-gray-800 font-bold">
            {isSubvideosJob ? 'รวมวิดีโอคลิป' : 'สร้างวิดีโอ'}
          </p>
        ...
        <div className="container mx-auto px-4 max-w-6xl">
                ...
                <div className="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-yellow-700 text-sm">
                      ...
                      {isSubvideosJob && ' (งานรวมคลิป - ไม่หัก Token)'}
                    </p>
                  </div>
          ...
          {/* 🔥 สำหรับ subvideos job - แสดงข้อความแจ้งเตือน */}
          {isSubvideosJob && status?.status === 'idle' && (
            <div className="text-center bg-purple-50 border-2 border-purple-200 rounded-xl p-6">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-purple-500 rounded-full mb-4">
                <span className="text-white text-3xl">🎬</span>
              </div>
              <h3 className="text-xl font-bold text-purple-800 mb-2">งานรวมคลิป</h3>
              <p className="text-purple-700">
                งานนี้จะเริ่มทำงานอัตโนมัติ ไม่ต้องกดปุ่มเริ่มสร้าง
                <br />
                <span className="text-sm">(ไม่มีการหัก Token)</span>
              </p>
            </div>
          )}
        </div>
...
//       const res = await fetch(`/api/status-unified?id=${fileId}&t=${Date.now()}`); // 🔥 เปลี่ยนตรงนี้
//       console.log('API status response:', res);
//       if (!res.ok) {
//         if (res.status === 404) {
//           setStatus({ 
