...
export default function CreateVideo() {
  ...
  // ลด refresh interval ลงเพื่อให้การอัปเดตสถานะเร็วขึ้น
    ...
    try {
      ...
      if (Array.isArray(data.clips)) {
        ...
        data.clips.forEach((c: Clip) => {
          if (c.finalVideo) setFinalVideo({ ...c });
          else if (c.video) newClips.push({ ...c });
        });
  ...
  async function startVideoCreation() {
    ...
    setLoading(true);
    setError(null);
    setStatus(prev => prev ? { ...prev, status: 'starting', updatedAt: new Date().toISOString() } : null);
    ...
    try {
      ...
      if (res.ok && result.executionId) {
        // หาก API เริ่ม workflow ได้สำเร็จ ให้ตั้งสถานะเป็น running ทันที
        setStatus(prev => prev ? { ...prev, executionId: result.executionId, status: 'running', updatedAt: new Date().toISOString() } : null);
      } else {
  ...
  async function cancelVideoCreation() {
    if (!id || !status?.executionId) return;

    setLoading(true);
    try {
      const res = await fetch('/api/cancel-wf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ executionId: status.executionId }),
      });
      const result = await res.json();

      if (res.ok) {
        setStatus(prev => prev ? { ...prev, status: 'idle', executionId: null, updatedAt: new Date().toISOString() } : null);
        setClips([]);
        setFinalVideo(null);
        setError(null);
      } else {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'idle': return <div className="w-3 h-3 rounded-full bg-gray-400"></div>;
      case 'starting':
      case 'running':
        return <div className="w-3 h-3 rounded-full bg-blue-500 animate-pulse"></div>;
      case 'succeeded': return <div className="w-3 h-3 rounded-full bg-green-500"></div>;
      ...
      <div className="min-h-screen">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-gray-800 mb-3">สร้างวิดีโอ</h1>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-6 border border-gray-200">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600 font-mono text-sm bg-gray-100 px-3 py-1 rounded-md inline-block">{id}</p>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {clips.map((clip, index) => (
                  <div key={index} className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
                    {clip.video && (
                      <video className="w-full h-40 object-cover" controls src={`${BASE_VIDEO_URL}${clip.video}`} />
                    )}
                    <div className="p-3">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-gray-500">คลิป {index + 1}</span>
                        <span className="text-xs text-gray-400">{clip.createdAt ? formatDateTime(new Date(clip.createdAt)) : '-'}</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {finalVideo && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl shadow-lg p-6 mb-6 border-2 border-green-200">
              ...
              <div className="bg-white rounded-lg overflow-hidden shadow-md">
                <video className="w-full max-h-96 object-contain" controls src={`${BASE_VIDEO_URL}${finalVideo.finalVideo}`} />
                <div className="p-4 bg-gray-50">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
                    <div>
                      <p className="font-semibold text-gray-800">วิดีโอพร้อมใช้งาน</p>
                      <p className="text-sm text-gray-600">สร้างเมื่อ: {finalVideo.createdAt ? formatDateTime(new Date(finalVideo.createdAt)) : '-'}</p>
                    </div>
                    <a href={`${BASE_VIDEO_URL}${finalVideo.finalVideo}`} download="final_video.mp4" className="inline-flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
          ...
          <div className="text-center mt-4 flex flex-col sm:flex-row justify-center items-center gap-4">
            ...
            {(status?.status === 'running' || status?.status === 'starting') && (
              <button
                onClick={cancelVideoCreation}
                className="inline-flex items-center space-x-2 px-6 py-4 rounded-xl font-bold text-lg bg-red-600 text-white shadow-lg hover:bg-red-700 transition-all duration-300"
              >
                <span>ยกเลิก</span>
              </button>
            )}
